...
/**
 * Service status EWS untuk Admin (Kepala Program Studi).
 *
 * Selalu ter-scope ke prodi milik admin yang sedang login.
 */
class StatusMahasiswaService
{
    ...
    /**
     * Distribusi jumlah mahasiswa per status EWS (lingkup prodi admin).
     *
    ...
    public function getDistribusiStatusEws(?int $tahunMasuk = null): array
    {
        $query = EarlyWarningSystem::query()
            ...
            ->where('mahasiswa.prodi_id', $this->getProdiId())
    ...
    /**
     * Detail lengkap 1 mahasiswa beserta status EWS terbaru (lingkup prodi admin).
     */
