...
use App\Services\Admin\Export\CapaianListMatakuliahExportService;
use App\Services\Admin\Export\CapaianMahasiswaGagalExportService;
use App\Services\Admin\Export\CapaianRataRataIpsExportService;
use App\Services\Admin\Export\CapaianTabelCapaianDetailExportService;
use App\Services\Admin\Export\CapaianTabelCapaianExportService;
use App\Services\Admin\Export\CapaianTopMatakuliahExportService;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;

/**
 * Capaian Mahasiswa untuk Admin (Kepala Program Studi).
 *
 * Mirror dari SuperFakultas\CapaianMahasiswaService, tapi selalu ter-scope
 * ke prodi milik admin yang sedang login (prodi_id di-inject ke setiap filter).
 */
class AdminCapaianMahasiswaService
{
    ...
    /**
     * Inject scope prodi admin ke filter.
     */
    private function scopeProdi(array $filters): array
    {
        $filters['prodi_id'] = $this->getProdiId();
    ...
    {
        $filters = $this->scopeProdi($filters);
        ...
        $query = DB::table('khs_krs_mahasiswa as khs')
            ...
            ->where('mahasiswa.prodi_id', $filters['prodi_id'])
    ...
    public function getRataRataIpsPerTahunProdi(array $filters = []): array
    {
        ...
        $query = DB::table('ips_mahasiswa as im')
            ...
            ->where('m.prodi_id', $filters['prodi_id'])
        ...
        $finalData = [];
        foreach ($groupedByProdi as $items) {
    ...
    public function getTabelCapaianMahasiswa(array $filters = []): array
    {
        ...
        $gagalByProdi = DB::table('khs_krs_mahasiswa as khs')
            ->join('mata_kuliahs as mk', 'khs.matakuliah_id', '=', 'mk.id')
            ->join('mahasiswa', 'khs.mahasiswa_id', '=', 'mahasiswa.id')
            ->join('prodis', 'mahasiswa.prodi_id', '=', 'prodis.id')
            ->where('khs.nilai_akhir_huruf', 'E')
        ...
        $ipsResults = DB::table('ips_mahasiswa as im')
            ->join('akademik_mahasiswa as akademik', 'im.mahasiswa_id', '=', 'akademik.mahasiswa_id')
            ->join('mahasiswa as m', 'im.mahasiswa_id', '=', 'm.id')
            ->join('prodis', 'm.prodi_id', '=', 'prodis.id')
        ...
        foreach ($ipsResults as $row) {
            ...
            if ($ipsTerakhir !== null && $ipsSebelum !== null) {
                if (! isset($ipsDataByProdi[$rowProdiId])) {
                    $ipsDataByProdi[$rowProdiId] = [
                        'kode_prodi' => $row->kode_prodi,
                        'nama_prodi' => $row->nama_prodi,
                ...
                $ipsDataByProdi[$rowProdiId]['total_ips_terakhir'] += $ipsTerakhir;
                $ipsDataByProdi[$rowProdiId]['total_ips_sebelum'] += $ipsSebelum;
                $ipsDataByProdi[$rowProdiId]['count']++;
            }
        }

        $sksResults = DB::table('akademik_mahasiswa as akademik')
        ...
        $formattedData = [];
        foreach ($ipsDataByProdi as $rowProdiId => $data) {
            $trenIPS = 'Stabil';
            $count = $data['count'];
            if ($count > 0) {
            ...
            $gagalData = $gagalByProdi->get($rowProdiId);
            $sksData = $sksResults->get($rowProdiId);
    ...
    public function getDetailTabelCapaianMahasiswa(array $filters = []): array
    {
        ...
        $gagalByKey = DB::table('khs_krs_mahasiswa as khs')
            ->join('mata_kuliahs as mk', 'khs.matakuliah_id', '=', 'mk.id')
            ->join('mahasiswa', 'khs.mahasiswa_id', '=', 'mahasiswa.id')
            ->join('prodis', 'mahasiswa.prodi_id', '=', 'prodis.id')
            ...
            ->groupBy('prodis.id', 'prodis.kode_prodi', 'prodis.nama', 'akademik.tahun_masuk')
            ->get()
            ->keyBy(fn ($item) => $item->prodi_id.'_'.$item->tahun_angkatan);
        ...
        foreach ($ipsDataByKey as $data) {
            ...
            $gagalData = $gagalByKey->get($data['prodi_id'].'_'.$data['tahun_angkatan']);
    ...
    public function getListMataKuliahPerProdi(array $filters = []): array
    {
        ...
        if ($groupByAngkatan) {
            $results = DB::table('khs_krs_mahasiswa as khs')
                ->join('mata_kuliahs as mk', 'khs.matakuliah_id', '=', 'mk.id')
                ->join('mahasiswa', 'khs.mahasiswa_id', '=', 'mahasiswa.id')
                ->join('prodis', 'mahasiswa.prodi_id', '=', 'prodis.id')
                ->join('akademik_mahasiswa as akademik', 'khs.mahasiswa_id', '=', 'akademik.mahasiswa_id')
            ...
            $formattedData = [];
            foreach ($groupedByKey as $matakuliahList) {
            ...
            $formattedData = [];
            foreach ($groupedByProdi as $matakuliahList) {
                $firstItem = $matakuliahList->first();
                $formattedData[] = [
                    'prodi' => [
                        'id' => (int) $firstItem->prodi_id,
    ...
    public function getListMahasiswaGagalPerMataKuliah(array $filters = []): array
    {
        ...
        $formattedData = [];
        foreach ($groupedByKey as $mahasiswaList) {
    ...
    /**
     * List mahasiswa yang punya nilai E (gagal) pada tahun angkatan tertentu,
     * beserta daftar mata kuliah yang gagal. Scope prodi admin.
     */
    public function getListMahasiswaGagalByAngkatan(array $filters = []): array
    {
        if (empty($filters['tahun_masuk'])) {
        ...
        $groupedByMahasiswa = $results->groupBy('mahasiswa_id');

        $formattedData = [];
        foreach ($groupedByMahasiswa as $matakuliahList) {
            $firstItem = $matakuliahList->first();
            $formattedData[] = [
                'mahasiswa_id' => (int) $firstItem->mahasiswa_id,
                'nim' => $firstItem->nim,
                'nama_mahasiswa' => $firstItem->nama_mahasiswa,
                'tahun_angkatan' => (int) $firstItem->tahun_angkatan,
                'jumlah_matakuliah_gagal' => $matakuliahList->count(),
                'matakuliah' => $matakuliahList->map(fn ($mk) => [
                    'matakuliah_id' => (int) $mk->matakuliah_id,
                    'kode_matakuliah' => $mk->kode_matakuliah,
                    'nama_matakuliah' => $mk->nama_matakuliah,
        ...
        return [
            'tahun_masuk' => (int) $filters['tahun_masuk'],
            'total_mahasiswa' => count($formattedData),
            'data' => $formattedData,
        ];
    }
    ...
    {
        return app(CapaianTopMatakuliahExportService::class)
            ->exportXlsx($filters);
    }

    public function exportRataRataIps($filters = [])
    {
        return app(CapaianRataRataIpsExportService::class)
            ->exportXlsx($filters);
    }

    public function exportTabelCapaian($filters = [])
    {
        return app(CapaianTabelCapaianExportService::class)
            ->exportXlsx($filters);
    }

    public function exportTabelCapaianDetail($filters = [])
    {
        return app(CapaianTabelCapaianDetailExportService::class)
            ->exportXlsx($filters);
    }

    public function exportListMatakuliah($filters = [])
    {
        return app(CapaianListMatakuliahExportService::class)
            ->exportXlsx($filters);
    }

    public function exportMahasiswaGagal($filters = [])
    {
        return app(CapaianMahasiswaGagalExportService::class)
            ->exportXlsx($filters);
    }
}
