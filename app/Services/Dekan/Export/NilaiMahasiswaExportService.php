...
class NilaiMahasiswaExportService
{
    ...
    private function writeSingleMahasiswa($sheet, $mhs, $startRow)
    {
        ...
        // Nilai D
        $this->writeSectionHeader($sheet, $startRow, 'MATA KULIAH NILAI D', 4);
        $startRow++;
        $this->writeHeaderRow($sheet, $startRow, $headers);
        $startRow++;
        foreach ($mhs->mata_kuliah_nilai_d as $i => $mk) {
            ...
            $this->styleDataRow($sheet, $startRow, 4, $i % 2 === 1);
            $startRow++;
        }
        ...
        // Nilai E
        $this->writeSectionHeader($sheet, $startRow, 'MATA KULIAH NILAI E', 4);
        $startRow++;
        $this->writeHeaderRow($sheet, $startRow, $headers);
        $startRow++;
        ...
        if (! empty($mhs->mk_nasional_kurang)) {
            $this->writeSectionHeader($sheet, $startRow, 'MK NASIONAL KURANG', 4);
            $startRow++;
    ...
    {
        return DB::table('mata_kuliahs')->whereIn('tipe_mk', ['nasional', 'fakultas'])->get()->groupBy('tipe_mk');
    }
    ...
    private function getMahasiswas($filters, $single)
    {
        ...
        if (! empty($filters['prodi_id'])) {
            $q->where('mahasiswa.prodi_id', $filters['prodi_id']);
        }
        if (! empty($filters['tahun_masuk'])) {
            $q->where('akademik_mahasiswa.tahun_masuk', $filters['tahun_masuk']);
        }
        if (! empty($filters['mahasiswa_id'])) {
            $q->where('mahasiswa.id', $filters['mahasiswa_id']);
        }
    ...
    private function getSummaryStats($f)
    {
        ...
        if (! empty($f['prodi_id'])) {
            $q->where('mahasiswa.prodi_id', $f['prodi_id']);
        }
        if (! empty($f['tahun_masuk'])) {
            $q->where('akademik_mahasiswa.tahun_masuk', $f['tahun_masuk']);
        }
    ...
    private function buildFilterDescription($f)
    {
        $d = [];
        if (! empty($f['prodi_id'])) {
            $d[] = 'Prodi: '.(Prodi::find($f['prodi_id'])->nama ?? $f['prodi_id']);
        }
        if (! empty($f['tahun_masuk'])) {
